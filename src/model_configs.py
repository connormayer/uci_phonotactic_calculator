...
def get_model_configs():
    ...
    for use_boundaries, smoothed, token_weighted, aggregation in product(
        [True, False], [False], [False], ["sum", "prod"]
    ):
    ...
    for use_boundaries, smoothed, token_weighted, aggregation in product(
        ...
        name = (
            "bi_nonpos_cond_" + ("wb" if use_boundaries else "nwb") + "_" + aggregation
        )
        if token_weighted:
            name += "_freq_weighted"
        if smoothed:
    ...
    for use_boundaries, smoothed, token_weighted, aggregation in product(
        ...
        name = (
            "bi_nonpos_joint_" + ("wb" if use_boundaries else "nwb") + "_" + aggregation
        )
        if token_weighted:
            name += "_freq_weighted"
        if smoothed:
