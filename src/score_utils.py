...
def generic_pos_unigram_score(token, pos_uni_freqs):
    """
    ...
    Returns:
      Total score (float) calculated as 1.0 plus the sum of position-specific probabilities.
    """
    total = 1.0
...
def generic_pos_bigram_score(token, pos_bi_freqs, conditional=False, use_word_boundaries=False):
    """
    ...
    Returns:
      Total score (float) calculated as 1.0 plus the sum of positional bigram probabilities.
    """
    if use_word_boundaries:
        token = [WORD_BOUNDARY] + token + [WORD_BOUNDARY]
    total = 1.0
