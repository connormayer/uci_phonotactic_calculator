...
def process_data(string_training_data, dev=True, training_split=60):
    ...
    inventory = ["<p>"] + [
        x for x in inventory if x != "<p>"
    ]  # ensure that the padding symbol is at index 0
...
def process_features(file_path, inventory):
    ...
    for line in file:
        ...
        if line[0] in inventory:
            feature_dict[line[0]] = [
                1.0 if x == "+" else 0.0 if x == "0" else -1.0 for x in line[1:]
            ]
            feature_dict[line[0]] += [0.0, 0.0, 0.0]

    num_feats = len(feature_dict[line[0]])

    feature_dict["<s>"] = [0.0 for x in range(num_feats - 3)] + [-1.0, -1.0, 1.0]
    feature_dict["<p>"] = [0.0 for x in range(num_feats - 3)] + [-1.0, 1.0, -1.0]
    feature_dict["<e>"] = [0.0 for x in range(num_feats - 3)] + [1.0, -1.0, -1.0]
