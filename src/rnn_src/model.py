...
class Feature_RNNLM(nn.Module):
    def __init__(
        self, params, feature_table
    ):  # the ith row of the feature table is the set of features for word i in phone2ix
        ...
        self.d_hid = params["d_hid"]
        self.i2R = nn.RNN(
            self.d_feats, self.d_hid, batch_first=True, num_layers=self.n_layers
        )  # input to recurrent layer, default nonlinearity is tanh
