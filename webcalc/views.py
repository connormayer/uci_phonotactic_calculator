...
class UploadTrainView(CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["documents"] = (
            UploadTrain.objects.all()
        )  # change to self.model.objects.all() ?
        return context
    ...
    def form_invalid(self, form):
        ...
        form.data["training_model"] = (
            ""  # reset training model dropdown selection in form
        )
    ...
    def form_valid(self, form):
        ...
        if not uploaded_training:
            if not default_training:
                ...
                train_file = join(
                    media_path,
                    "default",
                    basename((self.model.objects.last()).default_training_file),
                )
        else:
            train_file = join(
                media_path,
                "uploads",
                basename((self.model.objects.last()).training_file.name),
            )

        test_file = join(
            media_path, "uploads", basename((self.model.objects.last()).test_file.name)
        )
        ...
        if not train_success:
            messages.warning(
                self.request, "Invalid training file format: {}".format(message)
            )
            return self.form_invalid(form)
        test_success, message = util.valid_file(test_file)
        if not test_success:
            messages.warning(
                self.request, "Invalid test file format: {}".format(message)
            )
            return self.form_invalid(form)
        ...
        new_outfile_name = (
            old_outfile_name.replace(".csv", "") + "_" + test_file_name_sub + ".csv"
        )
...
class OutputView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["output_file"] = (
            new_outfile_name  # (self.model.objects.last()).out_file
        )
        return context
...
class UploadDefaultView(CreateView):
    ...
    def form_valid(self, form):
        ...
        train_file = join(
            media_path, "default", basename((self.model.objects.last()).training_file)
        )
        test_file = join(
            media_path, "uploads", basename((self.model.objects.last()).test_file.name)
        )
