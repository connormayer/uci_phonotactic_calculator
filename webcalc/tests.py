...
from src import io_utils, score_utils
    ...
    def testFitBigramsSmoothed(self):
        ...
        expected_probs = np.log(
            np.array(
                [
                    #    a_    t_    #_
                    [2 / 12, 4 / 12, 6 / 12],  # a→{a,t,#}
                    [7 / 10, 2 / 10, 1 / 10],  # t→{a,t,#}
                    [3 / 8, 4 / 8, 1 / 8],  # #→{a,t,#}
                ]
            )
        )
...
class TestNGramsTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.unigram_probs = fit_non_positional_unigram(self.token_freqs)
        self.bigram_probs = fit_bigram_conditional_non_positional_wb(
            self.token_freqs, self.unique_sounds
        )
        self.pos_unigram_freqs = fit_positional_unigrams(self.token_freqs)
        self.pos_bigram_freqs = fit_bigram_positional(self.token_freqs)
    ...
    def testGetBigramProbs(self):
        ...
        expected_prob = 0
        expected_prob += self.bigram_probs[hash_idx, t_idx]  # #→t
        expected_prob += self.bigram_probs[t_idx, a_idx]  # t→a
        expected_prob += self.bigram_probs[a_idx, t_idx]  # a→t
        expected_prob += self.bigram_probs[t_idx, a_idx]  # t→a (again)
        expected_prob += self.bigram_probs[a_idx, hash_idx]  # a→#
    ...
    def testGetPositionalUnigramScore(self):
        test_word = ["t", "a", "t", "a"]
        score = score_utils.generic_pos_unigram_score(test_word, self.pos_unigram_freqs)
        expected_score = 1
    ...
    def testGetPositionalBigramScore(self):
        test_word = ["t", "a", "t", "a"]
        score = score_utils.generic_pos_bigram_score(test_word, self.pos_bigram_freqs)
        expected_score = 1
