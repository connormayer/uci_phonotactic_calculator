...
# All models using explicit API names
from src.ngram_models import (
    fit_positional_unigrams,
    fit_non_positional_unigram,
...
class FitNGramsTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.unique_sounds = sorted(
            list(set([sound for token, _ in self.token_freqs for sound in token]))
        ) + ["#"]
    ...
    def testFitBigrams(self):
        """
        Test fit_bigram_conditional_non_positional_wb() with no smoothing, no weighting.
        (Conditional, non-positional bigram model.)
        """
        bigram_probs = fit_bigram_conditional_non_positional_wb(
    ...
    def testFitBigramsSmoothed(self):
        """
        Test fit_bigram_conditional_non_positional_wb() with smoothing, no weighting.
    ...
    def testFitBigramsWeighted(self):
        """
        Test fit_bigram_conditional_non_positional_wb() with token-weighted, no smoothing.
        (Conditional, non-positional.)
        """
        bigram_probs = fit_bigram_conditional_non_positional_wb(
        ...
        expected_probs = np.log(
            np.array(
                [
                    #     a_        t_       #_
                    [a_a / a_c, a_t / a_c, a_wb / a_c],  # a
                    [t_a / t_c, t_t / t_c, t_wb / t_c],  # t
    ...
    def testFitBigramsSmoothedWeighted(self):
        """
        Test fit_bigram_conditional_non_positional_wb() with token-weighted, and smoothing.
        (Conditional, non-positional.)
        """
        bigram_probs = fit_bigram_conditional_non_positional_wb(
    ...
    def testFitPositionalBigrams(self):
        """
        Test fit_bigram_positional() with no smoothing and no weighting.
        (Joint, positional bigram model without word boundaries.)
        """
        pos_bigram_freqs = fit_bigram_positional(self.token_freqs)
    ...
    def testFitPositionalBigramsWeighted(self):
        """
        Test fit_bigram_positional() with no smoothing, token-weighted.
        (Joint, positional without word boundaries.)
        """
        pos_bigram_freqs = fit_bigram_positional(self.token_freqs, token_weighted=True)
    ...
    def testFitPositionalBigramsSmoothed(self):
        """
        Test fit_bigram_positional() with smoothing, no weighting.
        (Joint, positional without word boundaries.)
        """
        pos_bigram_freqs = fit_bigram_positional(self.token_freqs, smoothed=True)
    ...
    def testFitPositionalBigramsSmoothedWeighted(self):
        """
        Test fit_bigram_positional() with smoothing and token weighting.
        (Joint, positional without word boundaries.)
        """
        pos_bigram_freqs = fit_bigram_positional(
            self.token_freqs, token_weighted=True, smoothed=True
        )
...
class TestNgramModelCombinations(unittest.TestCase):
    ...
    New tests to ensure every combination is present:
      ...
      - Non-positional bigrams: joint (via fit_bigram_joint_non_positional_*) vs. conditional (via fit_bigram_conditional_non_positional_*)
    ...
    def testPositionalBigrams_Conditional_NoWB(self):
        ...
        """
        pos_bigrams = fit_bigram_positional(
    ...
    def testNonPositionalBigrams_Conditional_WB(self):
        """
        Non-positional bigrams with conditional normalization computed by fit_bigram_conditional_non_positional_wb
        with word boundaries.
        For each previous symbol (row), the probabilities (after exponentiating the log)
        should sum to 1.
        """
        bp = fit_bigram_conditional_non_positional_wb(
            self.token_freqs, self.sounds_with_boundary
        )
        for row in range(bp.shape[0]):
            row_probs = np.exp(bp[row, :])
            total = np.sum(row_probs)
            self.assertAlmostEqual(total, 1.0, places=6)

    def testNonPositionalBigrams_Joint_WB(self):
        """
        Non-positional bigrams with joint normalization computed by fit_bigram_joint_non_positional_wb
        with word boundaries.
        For each previous symbol (row), the probabilities should sum to 1.
        """
        bp = fit_bigram_joint_non_positional_wb(self.token_freqs)
        for row in range(bp.shape[0]):
            row_probs = np.exp(bp[row, :])
            total = np.sum(row_probs)
            self.assertAlmostEqual(total, 1.0, places=6)
    ...
    def testNonPositionalBigrams_Conditional_NoWB(self):
        """
        Non-positional bigrams with conditional normalization computed by fit_bigram_conditional_non_positional_nwb
        without word boundaries.
        For each previous symbol, the probabilities should sum to 1.
        """
        bp = fit_bigram_conditional_non_positional_nwb(
            self.token_freqs, self.unique_sounds
        )
        for row in range(bp.shape[0]):
            row_probs = np.exp(bp[row, :])
    ...
    def testNonPositionalBigrams_Joint_NoWB(self):
        """
        Non-positional bigrams with joint normalization computed by fit_bigram_joint_non_positional_nwb
        without word boundaries.
        For each previous symbol, the probabilities should sum to 1.
        """
        bp = fit_bigram_joint_non_positional_nwb(self.token_freqs)
        for row in range(bp.shape[0]):
            row_probs = np.exp(bp[row, :])
            total = np.sum(row_probs)
            self.assertAlmostEqual(total, 1.0, places=6)
