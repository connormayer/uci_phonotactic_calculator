...
def assert_matrix_close(actual, expected, atol=1e-6):
    ...
    for i in range(rows):
        for j in range(cols):
            assert np.isclose(
                actual[i, j], expected[i, j], atol=atol
            ), f"Mismatch at cell row, col (prev, next) ({i}, {j}): {actual[i, j]} != {expected[i, j]}"
...
def test_fit_bigrams_conditional_WB(token_freqs, sound_index):
    ...
    expected_probs = log_expected(
        [[1 / 9, 3 / 9, 5 / 9], [6 / 7, 1 / 7, 0 / 7], [2 / 5, 3 / 5, 0 / 5]]
    )
...
def test_fit_bigrams_conditional_WB_smoothed(token_freqs, sound_index):
    ...
    model.fit(token_freqs, sounds)
    expected_probs = log_expected(
        [[2 / 12, 4 / 12, 6 / 12], [7 / 10, 2 / 10, 1 / 10], [3 / 8, 4 / 8, 1 / 8]]
    )
    assert_matrix_close(model.model_data, expected_probs)
...
def test_fit_bigrams_non_positional_joint_WB(token_freqs, sound_index):
    ...
    model.fit(token_freqs, sounds)
    expected_probs = log_expected(
        [[1 / 21, 3 / 21, 5 / 21], [6 / 21, 1 / 21, 0 / 21], [2 / 21, 3 / 21, 0 / 21]]
    )
    assert_matrix_close(model.model_data, expected_probs)
...
def test_fit_bigrams_non_positional_conditional_NWB(token_freqs, sound_index):
    ...
    expected_probs = log_expected([[1 / 4, 3 / 4], [6 / 7, 1 / 7]])
    assert_matrix_close(model.model_data, expected_probs)
...
def test_fit_bigrams_non_positional_joint_NWB(token_freqs, sound_index):
    ...
    model.fit(token_freqs, sound_index)
    expected_probs = log_expected([[1 / 11, 3 / 11], [6 / 11, 1 / 11]])
    assert_matrix_close(model.model_data, expected_probs)
...
def test_fit_bigram_nonpositional_score_sum(token_freqs, sound_index):
    ...
    t = sounds.index("t")
    a = sounds.index("a")
    # Order of bigrams with boundaries: (#, t), (t, a), (a, t), (t, a), (a, #). Rows = prev, Cols = next.
    assert score_val == sum(
        [
            1,
            np.exp(model.model_data[hash, t]),  # # → t
            np.exp(model.model_data[t, a]),  # t → a
            np.exp(model.model_data[a, t]),  # a → t
            np.exp(model.model_data[t, a]),  # t → a
            np.exp(model.model_data[a, hash]),  # a → #
        ]
    )
...
def test_fit_bigram_nonpositional_score_prod(token_freqs, sound_index):
    ...
    assert score_val == sum(
        [
            model.model_data[hash, t],  # # → t
            model.model_data[t, a],  # t → a
            model.model_data[a, t],  # a → t
            model.model_data[t, a],  # t → a
            model.model_data[a, hash],  # a → #
        ]
    )
