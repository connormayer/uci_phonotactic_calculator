...
def test_run_calculator_integration(dummy_train_file, dummy_test_file, tmp_path):
    ...
    ), f"Header mismatch: {rows[0]} != {expected_header}"

    # Verify each result row has the same number of columns as the header.
    for row in rows[1:]:
        assert len(row) == len(
            expected_header
        ), f"Row length {len(row)} does not match header length {len(expected_header)}"
        ...
        for cell in row[2:]:
            try:
                ...
                assert np.isfinite(val) or val == float(
                    "-inf"
                ), f"Score {cell} is not finite or -inf"
            except ValueError:
                # If conversion fails, the cell should be empty.
                assert (
                    cell == ""
                ), f"Expected empty string for non-numeric cell, got {cell}"
